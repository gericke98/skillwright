...
## bskill prune — bound screenshot disk growth
- **What:** A `bskill prune` command that drops per-step screenshots for old/superseded skill versions while keeping `assets/recording.json` (immutable evidence) intact.
...
- **Pros:** Keeps `~/.browser-skills/` bounded; predictable footprint.
...
- **What:** A tab-ownership / session-lock model for when `bskill run` drives the user's live profile while the user may also be using it.
...
- **What:** A reconciliation story for when `bskill install` falls back from symlink to copy (Windows, restricted FS).
...
- **Context:** Codex outside-voice finding, /plan-eng-review 2026-07-06. At minimum, `bskill list`/`bskill install` should detect copy-mode installs and flag them as stale-able, with a `bskill sync` to refresh.
