...
# `skillwright run <slug>`. This script proves the mechanism end-to-end from the
...
# 1. Build the publishable bundle and expose `skillwright` on PATH via a shim
#    (this stands in for `npm i -g skillwright`).
pnpm --filter skillwright bundle >/dev/null
WORK="$(mktemp -d)"; trap 'rm -rf "$WORK"' EXIT
BIN="$WORK/bin"; mkdir -p "$BIN"
cat > "$BIN/skillwright" <<SHIM
#!/bin/sh
exec node "$REPO/packages/cli/bundle/bin.js" "\$@"
SHIM
chmod +x "$BIN/skillwright"
export PATH="$BIN:$PATH"
export SKILLWRIGHT_HOME="$WORK/lib"

# 2. Distill a fixture recording into a skill (as the shipped CLI).
cat > "$WORK/rec.json" <<'JSON'
{"title":"Delete an invoice","steps":[{"type":"navigate","url":"https://erp.test/invoices"},{"type":"change","selectors":[["aria/Invoice number"]],"value":"INV-1"},{"type":"click","selectors":[["aria/Delete"]]}],"x-skillwright":{"version":1,"segment":{"id":"s","parentSkill":null,"recordedAt":"2026-07-07"}}}
JSON
skillwright distill "$WORK/rec.json" >/dev/null
SLUG="delete-an-invoice"

# 3. Install into a fresh project's cross-agent skill roots.
PROJ="$WORK/proj"; mkdir -p "$PROJ"
skillwright install "$SLUG" --project "$PROJ" >/dev/null
...
echo "PASS script-mode dispatch: skillwright run reached the replay path"
